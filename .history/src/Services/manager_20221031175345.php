...
Class Manager
{
        ...
        foreach($data as $key=>$value){
            ...
            if('Id'!==$attr){           //El Id nunca debe modificarse
                ...
                elseif(method_exists($entity,$addmethod)){
                    dump($value);
                    $value=$this->array_to_object($value,'form_planta',$attr->getval);
                    // foreach($value as $element)
                    //     $valor=$this->array_to_object($value,'form_planta');
    ...
    $normalizer = new ObjectNormalizer($classMetadataFactory);
    $serializer = new Serializer([$normalizer]);
    return $serializer->denormalize($array,$entity::class,null,['groups' =>$groups]);
