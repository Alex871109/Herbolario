...
Class Manager
{
        ...
        foreach($data as $key=>$value){
            ...
            if('Id'!==$attr){           //El Id nunca debe modificarse
                ...
                elseif(method_exists($entity,$addmethod)){
                    $value=$this->array_to_object($value,$entity,'form_planta');
                    dump($value); die;
    ...
    public function array_to_object($array,$object_type,$groups)
    {
    $classMetadataFactory = new ClassMetadataFactory(new AnnotationLoader(new AnnotationReader()));
    $normalizer = new ObjectNormalizer($classMetadataFactory);
    $serializer = new Serializer([$normalizer]);
    return $serializer->denormalize($array,$object_type,null,['groups' =>$groups]);
