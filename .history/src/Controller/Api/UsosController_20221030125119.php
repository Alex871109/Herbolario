...
class UsosController extends AbstractController
{
    ...
    {
        $usos=$usosRepository->findOneById($id);
        if(!$usos)
       ...
       if($request->getMethod()==='POST'){
            ...
            if($save_operation['error'])
                $dataResponse = ['status' => 500, 'response' => 'fail','method'=>'POST', 'usos' => null];
            else
                $dataResponse=['status'=>200,'response'=>'success', 'method'=>'POST','usos'=>$manager->object_to_array($save_operation['entity'],'basic')];
