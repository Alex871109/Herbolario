...
class UsosController extends AbstractController
{
    ...
    public function delete(UsosRepository $usosRepository,PlantaRepository $infocomercialRepository, Manager $manager, int $id): JsonResponse
    {
        $uso=$usosRepository->findOneById($id);
        if(!$uso)
            throw $this->createNotFoundException('Uso not found');
        ...
        try {
            $plantas=$infocomercialRepository->findByHerbolarioid($herbolario);
