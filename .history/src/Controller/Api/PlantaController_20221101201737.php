...
class PlantaController extends AbstractController
{
    ...
    public function edit_get(Request $request,int $id,EntityManagerInterface $entityManager, Manager $manager, PlantaRepository $plantaRepository): JsonResponse
    {
        ...
        dump($planta);
        dump($manager->object_to_array($planta,'form_planta')); die;
       if($request->getMethod()==='POST'){
            ...
            // else
                ...
                $dataResponse=['status'=>200,'response'=>'success', 'method'=>'POST','planta'=>$manager->object_to_array($save_operation,'form_planta')];
        }

            // dump($save_operation['entity']);
            // dump($manager->object_to_array($save_operation['entity'],'form_planta')); die;
