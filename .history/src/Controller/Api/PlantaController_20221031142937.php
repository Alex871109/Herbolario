...
class PlantaController extends AbstractController
{
    ...
    public function edit_get(Request $request,int $id,EntityManagerInterface $entityManager, Manager $manager, PlantaRepository $plantaRepository): JsonResponse
    {
        ...
        if(!$planta)
            throw $this->createNotFoundException('P not found');
        $dataResponse=['planta'=>$manager->object_to_array($planta,'basic')];
       ...
       if($request->getMethod()==='POST'){
            ...
            if($save_operation['error'])
                ...
                $dataResponse=['status'=>200,'response'=>'success', 'method'=>'POST','uso'=>$manager->object_to_array($save_operation['entity'],'form_planta')];
