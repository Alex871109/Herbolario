    ...
    public function nueva(Request $request,UsosRepository $usosRepository,HerbolarioRepository $herbolarioRepository, EntityManagerInterface $entityManager): Response
    {
        ...
        if($request->getMethod()==='POST'){
            ...
            for($i=1;$i<4;$i++){
                ...
                $herbolario_array[$i]=$herbolarioRepository->findOneBy(['nombre' =>$nombre_herbolario]); //Si no hago el findOneBy ve como q devuelve 1 array y luego no deja usar ->get
                ...
                $herbolario_id=$herbolario_array[$i]->getId();
                dump($herbolario_id);
                die();
